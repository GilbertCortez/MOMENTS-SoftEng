...
router.get('/sales', (req,res)=>{
    ...
    db.query(`Select count(*) as c from
(Select tblRental.inttransactionNo AS transactions from tblRental join tblitem
	on tblRental.intItemno = tblItem.intItemno join tblProvider
	on tblitem.intProviderID = tblProvider.intProviderID
	where tblProvider.intProviderID = ${req.session.user.intProviderID}
UNION ALL
Select tblservices.inttransactionNo AS transactions from tblservices join tblgenservice
	on tblservices.intgenserviceno = tblgenservice.intgenserviceno join tblProvider
	on tblgenservice.intProviderID = tblProvider.intProviderID
	where tblProvider.intProviderID = ${req.session.user.intProviderID}) as temp;
	`, (err,results,fields)=>{
    if (err) console.log(err);

    var profile = req.session.user;
    res.render('businessman/views/sales', {count: results[0].c, selected: 0, profile: profile, user: `${req.session.user.strProviderFName}`+" "+ `${req.session.user.strProviderLName}`  });
...
(Select tblRental.inttransactionNo AS transactions from tblRental join tblitem
	on tblRental.intItemno = tblItem.intItemno join tblProvider
	on tblitem.intProviderID = tblProvider.intProviderID
	where tblProvider.intProviderID = ${req.session.user.intProviderID} and (MONTH(tblRental.dtmRentalDate) = ${month} and YEAR(tblRental.dtmRentalDate) = ${year})
UNION ALL
Select tblservices.inttransactionNo AS transactions from tblservices join tblgenservice
	on tblservices.intgenserviceno = tblgenservice.intgenserviceno join tblProvider
	on tblgenservice.intProviderID = tblProvider.intProviderID
	where tblProvider.intProviderID = ${req.session.user.intProviderID} and (MONTH(tblServices.dtmServiceDate) = ${month} and YEAR(tblServices.dtmServiceDate) = ${year})) as temp;
	`, (err,results,fields)=>{
    if (err) console.log(err);

    var profile = req.session.user;
    res.render('businessman/views/sales', {selected: 1, month: `${req.body.month}`, year:`${req.body.year}`, profile: profile, user: `${req.session.user.strProviderFName}`+" "+ `${req.session.user.strProviderLName}`, count: results[0].c
    });
  });
...
router.get('/payment', (req,res)=>{
      db.query(`Select * from tblpayment join tbltransaction on tblpayment.intTransactionNo = tbltransaction.intTransactionNo join tblevent on tblevent.intEventNo = tbltransaction.intEventNo join tblorganizer on tblevent.intOrganizerID = tblorganizer.intOrganizerID
    join tblServices on tblServices.intTransactionNo = tblTransaction.intTransactionNo
    join tblRental on tblRental.intTransactionNo = tblTransaction.intTransactionNo
    join tblGenService on tblGenservice.intGenServiceNo = tblServices.intGenServiceNo
    join tblItem on tblItem.intItemNo = tblRental.intItemNo
    where tblitem.intProviderID = ${req.session.user.intProviderID} or tblGenService.intProviderID = ${req.session.user.intProviderID}`, (err,results,fields)=>{
      if (err) console.log(err);

      var profile = req.session.user;
      res.render('businessman/views/payments', { profile: profile, user: `${req.session.user.strProviderFName}`+" "+ `${req.session.user.strProviderLName}`, content: results
      });
    });
});
